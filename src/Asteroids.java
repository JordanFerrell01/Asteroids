...
public class Asteroids extends Applet implements Runnable, KeyListener {
	...
	//creat the identity transform(0.0)
	AffineTransform identity = new AffineTransform();
	
	//create a random number generator
	Random rand = new Random();
	
	//spin left or spin right tracker
	boolean bSpinLeft = false, bSpinRight = false;
	...
	public void init() {
		...
		for (int n = 0; n<ASTEROIDS; n++) {
			...
			ast[n].setY((double)rand.nextInt(640)+20);
	...
	public void update(Graphics g) {
		...
		g2d.drawString("Move angle: " 
				+ Math.round(ship.getMoveAngle()) + 90, 5, 25);
...
//	 THE PAINT EVENT OCCURS WHEN THE APPLET WINDOW NEEDS TO BE REFRESHED
//	 THIS METHOD IS CALLED BY THE UPDATE METHOD AND SIMPLY SERVES THE
//	 PURPOSE OF DRAWING THE BACK BUFFER TO THE APPLET WINDOW.
...
//	There are three thread events that are part of a program when you implement
//	the Runnable interface in a Java applet. Runnable tells Java that your applet will
//	support more than one thread. A thread is sort of a mini program that can run
//	on its own. You create a new thread in the start() event, and then destroy that
//	thread in the stop() event to keep things running smoothly.	
	
	//thread start event - start the game loop running
	public void start() {
		//create the game loop thread for real-time updates
	...
	public void updateBullets() {
		...
		for(int n = 0; n < BULLETS; n++) {
			...
			if (bullet[n].isAlive()) {
				...
				//bullet disapperas at left/right edge
